...
exports.login = catchAsync(async (req, res, next) => {
    ...
    //3) if every thing is ok, send token to lient
...
exports.forgotPassword = catchAsync( async (req, res, next) => {
  ...
  try {
    ...
    return next (new AppError("There was a problem sending Email. try again later", 500)
    );
...
exports.updateMyPassword = catchAsync(async (req, res, next) =>{
    //1) Get user from current collection
const user = await User.findById(req.user.id).select("+password");


    //2)check if Posted current password is currect
if(!(await user.correctPassword(req.body.passwordCurrent, user.password))) {
    return next(new AppError("Your current passowrd is wrong", 400));
}
